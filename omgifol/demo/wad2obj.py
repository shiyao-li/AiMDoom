...
# python
import math, argparse, os, sys, glob
...
def _polygons_with_line_definitions(edit, vi, vertexes, textureSizes, polys):
    for line in edit.linedefs:
        ...
        if line.front != -1:
            ...
            #print(list(textureSizes.keys()))  # 打印所有的键
...
def writemtl(wad):
    ...
    for name,texture_definition in t.items():
        ...
        names.append(name)
    """textureSizes = {}
    for image_path in glob.glob('/home/clementin/Dev/DOOM/omgifol/demo/textures/*.png'):
        image = Image.open(image_path)
        textureSizes[image_path.split('/')[-1].strip('.png')] = image.size"""
...
def parse_args():
    ...
    epilog = "Example: wad2obj.py -m 'E1*' -o /tmp"

    parser = argparse.ArgumentParser(description=__doc__, epilog=epilog)
    parser.add_argument(
            '-s', '--source', type=str, required=True,
            help='Path to the directory containing input WAD files.')
    parser.add_argument(
...
def main():
    ...
    source_directory = args.source  # Use the source directory from command line
    output_dir = args.output  

    for wad_file in glob.glob(os.path.join(source_directory, '*.wad')):
        os.chdir(original_directory)
        wad_name = os.path.basename(wad_file)
        wad_base_name = os.path.splitext(wad_name)[0]

        print "Loading %s..." % wad_file
        inwad = wad.WAD()

        # Check if file exists before attempting to open, if not print error and continue with next file
        if not os.path.exists(wad_file):
            print "Error: The file %s does not exist." % wad_file
            continue

        inwad.from_file(wad_file)

        inwad_doom2 = wad.WAD()
        inwad_doom2.from_file('./DOOM2.WAD')
        ...
        current_output_dir = os.path.join(output_dir, wad_base_name.lower())
        ...
        try:
            textureNames, textureSizes = writemtl(inwad_doom2)
            ...
            print "Error processing %s: Missing texture %s" % (wad_file, str(e))
            continue 
